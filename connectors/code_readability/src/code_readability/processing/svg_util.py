...
def update_svg_size(svg_tree: etree.ElementTree, width=None, height=None) -> etree.ElementTree:
...
def remove_by_xpath(svg_tree: etree.ElementTree, xpath: str):
    for bad in svg_tree.xpath(xpath):
        bad.getparent().remove(bad)


def export_image(svg_tree: etree.ElementTree, path: Path):
...
def export_image2(svg_tree: etree.ElementTree, path: Path, size=(128, 128)):
    with tempfile.NamedTemporaryFile() as f:
